...
const DataService = {
    ...
    async updateUser(id, userDetails) {
        ...
        const index = users.findIndex(u => parseInt(u.id) === numId);
        if (index !== -1) {
            ...
            tasks.forEach(task => {
                if (task.user && parseInt(task.user.id) === numId) {
    ...
    async deleteUser(id) {
        ...
        const numId = parseInt(id);
        users = users.filter(u => parseInt(u.id) !== numId);
        this._saveUsers(users);
    ...
    async createTask(task) {
        ...
        if (task.user && task.user.id) {
            const users = this._getUsers();
            const userId = parseInt(task.user.id);
            fullUser = users.find(u => parseInt(u.id) === userId) || null;
        }
    ...
    async deleteTask(id) {
        ...
        const numId = parseInt(id);
        tasks = tasks.filter(t => parseInt(t.id) !== numId);
        this._saveTasks(tasks);
    }
};
